...
MAKE_AUTO_HOOK_MATCH(CutScoreBuffer_Init, &CutScoreBuffer::Init, bool, CutScoreBuffer* self, by_ref<NoteCutInfo> noteCutInfo) {
...
static void AddSignalUpdates(UnityEngine::Component* self, std::function<void()> disable, std::function<void()> enable) {
    Engine::SetOnDisable(self, disable);
    ...
    Engine::SetOnEnable(self, enable);
    enable();
}
...
MAKE_AUTO_HOOK_MATCH(
    ...
    if (!cell->interactable) {
        cell->enabled = false;
        if (auto cast = cell.try_cast<HMUI::IconSegmentedControlCell>())
